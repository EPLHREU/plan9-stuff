...
#define BACK 0x1c2024FF
#define ACTIVE 0xa24ed1FF
#define INACTIVE 0x282c30FF
...
main(int argc, char **argv)
{
	...
	if(initdraw(0, fontname, "winwatch") < 0)
		sysfatal("initdraw: %r");
	lightblue = allocimagemix(display, INACTIVE, INACTIVE);

	statecol[0] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, INACTIVE);
	statecol[1] = lightblue;
	statecol[2] = lightblue;
	statecol[3] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, ACTIVE);
